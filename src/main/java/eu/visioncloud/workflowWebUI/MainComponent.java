...
public class MainComponent extends CustomComponent {
	...
	private InjectTab hl_injectTab;
	@AutoGenerated
	 ...
	 * editor.
	 */
	public MainComponent(String t, AuthToken auth, ClientInterface oClient) {
	...
	private TabSheet buildMainTab() {
		...
		// hl_inputTab
		hl_injectTab = new InjectTab(tenant, auth, oClient);
		hl_inputTab = buildHl_inputTab();
		mainTab.addTab(hl_injectTab, "Operation", null).setClosable(false);
		mainTab.addTab(hl_inputTab, "Analysis", null).setClosable(false);
	...
	private Set<HandlerInfo> LoadHandlers() {
		...
		try {
			...
			logger.error("load outputs failed ", je);
		}
		return handlers;
	}
